...
ESPColor ESPColor::operator*(const uint8_t scale) const {
  return ESPColor(
      esp_scale8(this->red, scale),
      esp_scale8(this->green, scale),
      esp_scale8(this->blue, scale),
...
ESPColor ESPColor::operator+(const ESPColor &add) const {
  ESPColor ret;
  if (uint8_t(add.r + this->r) < this->r) ret.r = 255;
  else ret.r = this->r + add.r;
  if (uint8_t(add.g + this->g) < this->g) ret.g = 255;
  else ret.g = this->g + add.g;
  if (uint8_t(add.b + this->b) < this->b) ret.b = 255;
  else ret.b = this->b + add.b;
  if (uint8_t(add.w + this->w) < this->w) ret.w = 255;
  else ret.w = this->w + add.w;
  return ret;
}

ESPColor &ESPColor::operator+=(const ESPColor &add) {
  return *this = (*this) + add;
}

ESPColor ESPColor::operator+(uint8_t add) const {
  return (*this) + ESPColor(add, add, add, add);
}

ESPColor &ESPColor::operator+=(uint8_t add) {
  return *this = (*this) + add;
}

ESPColor ESPColor::operator-(const ESPColor &subtract) const {
  ESPColor ret;
  if (subtract.r > this->r) ret.r = 0;
  else ret.r = this->r - subtract.r;
  if (subtract.g > this->g) ret.g = 0;
  else ret.g = this->g - subtract.g;
  if (subtract.b > this->b) ret.b = 0;
  else ret.b = this->b - subtract.b;
  if (subtract.w > this->w) ret.w = 0;
  else ret.w = this->w - subtract.w;
  return ret;
}

ESPColor &ESPColor::operator-=(const ESPColor &subtract) {
...
ESPColor &ESPColor::operator-=(uint8_t subtract) {
  return *this = (*this) - subtract;
}
...
ESPColor ESPColor::operator*(const ESPColor &scale) const {
  return ESPColor(
      esp_scale8(this->red, scale.red),
      esp_scale8(this->green, scale.green),
      esp_scale8(this->blue, scale.blue),
      esp_scale8(this->white, scale.white)
  );
}

uint8_t &ESPColor::operator[](uint8_t x) {
  return this->raw[x];
}

bool ESPColor::is_on() {
  return this->r != 0 || this->g != 0 || this->b != 0 || this->w != 0;
}
...
ESPColorView::ESPColorView(uint8_t *red, uint8_t *green, uint8_t *blue, uint8_t *white, uint8_t *effect_data,
    ...
    : red_(red), green_(green), blue_(blue), white_(white), effect_data_(effect_data),
...
void ESPColorView::set(const ESPColor &color) const {
  this->set_rgbw(color.r, color.g, color.b, color.w);
}

void ESPColorView::set_red(uint8_t red) const {
...
void ESPColorView::set_blue(uint8_t blue) const {
...
ESPColor ESPColorCorrection::color_correct(ESPColor color) const {
  ...
  return ESPColor(
      this->color_correct_red(color.red),
      this->color_correct_green(color.green),
      this->color_correct_blue(color.blue),
      this->color_correct_white(color.white)
  );
}
...
ESPColor ESPColorCorrection::color_uncorrect(ESPColor color) const {
  ...
  return ESPColor(
      this->color_uncorrect_red(color.red),
      this->color_uncorrect_green(color.green),
      this->color_uncorrect_blue(color.blue),
      this->color_uncorrect_white(color.white)
  );
}
